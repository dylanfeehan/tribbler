...
func RunServer(host, port string) {
	...
	grpcServer := grpc.NewServer(serverOptions...) // Server is an interface we can register multiple grpc server implementations to
	...
	fmt.Println("Starting to serve trib server")
